...
import Notification , {NEW, DEL, MOD} from './notification';
...
class Http {
  ...
  putWishlist = (wishlist, name) => {
    fetch('http://localhost:3001/wishlist', {
       method: 'PUT', headers: {'Content-Type': 'application/json'},
       body: JSON.stringify({wishlistId: wishlist._id, title: name})
     }).then(response => notif.post(MOD, name))
  }

  deleteWishlist = wishlist => {
    fetch('http://localhost:3001/wishlist', {
       method: 'DELETE', headers: {'Content-Type': 'application/json'},
       body: JSON.stringify({wishlistId: wishlist._id})
     }).then(response => notif.post(DEL, wishlist))
  }
